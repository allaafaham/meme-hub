...
def profile(request, prof=None):
    """
    User profile view for updating profile
      information and viewing user's memes.
...
def meme_create(request):
    ...
    if request.method == "POST":
        ...
        form = MemeForm()
    return render(
        request, "core/meme_form.html", {"form": form, "title": "Create Meme"}
    )
...
def meme_detail(request, pk):
    ...
    if request.method == "POST" and request.user.is_authenticated:
        ...
        if comment_form.is_valid():
            ...
            if (last_comment and
                    (timezone.now() - last_comment.created_at).seconds < 60):
                messages.warning(
                    request,
                    "This comment was already posted. Please wait a moment "
...
def comment_delete(request, pk):
    ...
    return render(
        request,
        "core/comment_confirm_delete.html",
        {"comment": comment}
    )
...
def meme_update(request, pk):
    ...
    return render(
        request, "core/meme_form.html", {"form": form, "title": "Update Meme"}
    )
...
def meme_delete(request, pk):
    ...
    if meme.user != request.user:
        return HttpResponseForbidden(
            "You don't have permission to delete this meme."
        )
